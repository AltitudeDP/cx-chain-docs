...
CX Chain uses **Verifiable Random Function (VRF)** to provide a secure and transparent source of on-chain randomness, ensuring that all game outcomes are fair and unpredictable. This service operates through a cross-chain communication process between CX Chain and the Avalanche C-Chain.
...
*   **VRF Provider (on Avalanche C-Chain):** This contract receives requests from the CX Chain proxy and communicates directly with VRF to generate the random value.
...
3.  **VRF Interaction:** The `VRF Provider` submits the request to VRF for secure generation of a random number.
4.  **Cross-Chain Return:** VRF returns the verifiable random number to the `VRF Provider`, which then uses Teleporter to send it back to the `VRF Proxy` on CX Chain.
...
sequenceDiagram
    ...
    participant VRFSC as VRF Service (Avalanche C-Chain)
    ...
    VRFProvider->>+VRFSC: Submits request to VRF
    
    %% VRF generates the random number off-chain and sends it back in a separate transaction %%
    
    VRFSC-->>-VRFProvider: Returns verifiable random words via callback
